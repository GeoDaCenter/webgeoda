...
const aggregateDataFunction = (
  ...
  if (operation === "weighted_average") {
    for (let i = 0; i < geoids.length; i++) {
      ...
      totalPopulation += selectionPop;
      dataArray.push(
        dataFn(numeratorTable[geoids[i]], denominatorTable[geoids[i]], params) *
          selectionPop
      );
    }
  } else {
...
export const generateReport = (
  ...
  if (state.storedData.hasOwnProperty(currentTables.testing?.file)) {
    report.wk_pos = aggregateTimeseries(
    ...
    report.testing = aggregateTimeseries(
    ...
    report.ccpt = aggregateTimeseries(
...
export const findDataset = (currentData, preset) =>
  find(preset, (d) => d.geodata === currentData);
...
export const findAnyTable = (tableName, presets) => {
  for (let i = 0; i < presets.length; i++) {
    try {
      return findDatasetTable(presets[i].geodata, tableName, presets);
    } catch {}
  }
};
export const findDatasetWithTable = (tableName, presets) => {
  for (let i = 0; i < presets.length; i++) {
    if (presets[i].tables?.hasOwnProperty(tableName)) return presets[i].geodata
  }
};
...
export const getTable = (currentData, storedData, tableName, presets) =>
  storedData[findTable(currentData, tableName, presets)?.file]?.data || {};
...
export const parseTooltipData = (geoid, state, presets) => {
  ...
  for (let i = 0; i < variables.length; i++) {
    ...
    if (!variables[i].nIndex && !variables[i].nProperty) {
      ...
      numeratorData = getTable(
        state.currentData,
        state.storedData,
        variables[i].numerator,
        datasets
      )[geoid];
    }
    ...
    if (!variables[i].dIndex && !variables[i].dProperty) {
      denominatorData =
        state.storedGeojson[state.currentData].properties[geoid];
      cleanVariable.dProperty = cleanVariable.denominator;
    } else {
      denominatorData = getTable(
        state.currentData,
        state.storedData,
        variables[i].denominator,
        datasets
      )[geoid];
    }

    const parsedVariable = dataFn(
      numeratorData,
      denominatorData,
      cleanVariable
    );
