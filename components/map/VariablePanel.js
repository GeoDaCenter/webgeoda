...
export default function VariablePanel(props){
    ...
    return (
        <div className={styles.panelContainer + ' ' + (props.open ? styles.open : styles.hidden)}>
            <Listbox 
                value={currentVariable}
                onChange={(value) => dispatch({type:'CHANGE_VARIABLE', payload:value})}
            >
                {dataPresets.variables.map((entry, i) => 
                    <ListboxOption value={entry.variable} key={`variable-list-${i}`}>{entry.variable}</ListboxOption>
                )}
            </Listbox>
        </div>
    )
